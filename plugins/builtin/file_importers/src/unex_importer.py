...
def _load_unex1x(path: Path, logs: list) -> Node:
        ...
        for line_number, line in enumerate(input_file):
            ...
            if "> Cartesian coordinates of all atoms (Angstroms) in" in line:
                ...
                if molecule_name in molecules:
                    ...
                    molecule = Node(name=molecule_name, type="builtin.molecule")
...
def _load_unex2x(path: Path, logs: list) -> Node:
    result = Node(name=path.name, type="builtin.unex")
...
def read(path: Path, logs: list[str]) -> Node:
    ...
    """
    version = _get_format_version(path)

    # UNEX 1.x
    if version < 2000000:
        result = _load_unex1x(path, logs)
    # UNEX >= 2.x
    else:
        result = _load_unex2x(path, logs)
