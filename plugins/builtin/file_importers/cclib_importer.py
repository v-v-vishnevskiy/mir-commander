...
def read(path: Path, logs: list[str]) -> Node:
    ...
    result = Node(name=path.name, type="builtin.molecule", data=molecule)
    ...
    if hasattr(data, "atomcoords"):
        ...
        if cshape[0] > 1:
            ...
            if hasattr(data, "optstatus"):
                optcg_item = Node(name="Optimization", type="builtin.atomic_coordinates_group")
            ...
            # Otherwise this is an undefined set of coordinates
                ...
                molcg_item = Node(name="Coordinates", type="builtin.atomic_coordinates_group")
    ...
    if hasattr(data, "scancoords"):
        ...
        if cshape[0] > 0:
            scancg_item = Node(name="Scan", type="builtin.atomic_coordinates_group")
