...
class Appearance(ControlBlock):
    ...
    def update_values(self, program: "Program"):
        ...
        for style in program.config.styles:
            if self._style_combobox.findText(style.name) < 0:
                self._style_combobox.addItem(style.name)
        self._style_combobox.setCurrentText(program.visualizer.current_style.name)
