...
class ContextMenu(QMenu):
    ...
    def _init_actions(self):
        save_img_act = QAction(self.tr("Save image..."), self.parent())
        ...
        next_style_act = QAction(self.tr("Next style"), self.parent())
        ...
        prev_style_act = QAction(self.tr("Previous style"), self.parent())
        ...
        projection_act = QAction(self.tr("Toggle projection"), self.parent())
    ...
    def _init_atom_labels_menu(self):
        menu = QMenu(self.tr("Atom labels"))
        self.addMenu(menu)

        show_all_act = QAction(self.tr("Show all"), self.parent())
        show_all_act.setStatusTip(self.tr("Show labels for all atoms"))
        show_all_act.triggered.connect(self._visualizer.atom_labels_show_for_all_atoms)
        menu.addAction(show_all_act)

        hide_all_act = QAction(self.tr("Hide all"), self.parent())
        hide_all_act.setStatusTip(self.tr("Hide labels for all atoms"))
        hide_all_act.triggered.connect(self._visualizer.atom_labels_hide_for_all_atoms)
        menu.addAction(hide_all_act)

        menu.addSeparator()

        show_for_selected_atoms_act = QAction(self.tr("Show selected"), self.parent())
        show_for_selected_atoms_act.setStatusTip(self.tr("Show labels for selected atoms"))
        ...
        hide_for_selected_atoms_act = QAction(self.tr("Hide selected"), self.parent())
        hide_for_selected_atoms_act.setStatusTip(self.tr("Hide labels for selected atoms"))
        ...
        toggle_all_act = QAction(self.tr("Toggle all"), self.parent())
        toggle_all_act.setShortcut(QKeySequence(self._keymap.toggle_labels_visibility_for_all_atoms))
        toggle_all_act.setStatusTip(self.tr("Toggle labels for all atoms"))
        ...
        toggle_selected_act = QAction(self.tr("Toggle selected"), self.parent())
        toggle_selected_act.setShortcut(QKeySequence(self._keymap.toggle_labels_visibility_for_selected_atoms))
        toggle_selected_act.setStatusTip(self.tr("Toggle labels for selected atoms"))
    ...
    def _init_bonds_menu(self):
        bonds_menu = QMenu(self.tr("Bonds"))
        self.addMenu(bonds_menu)

        add_selected_act = QAction(self.tr("Add selected"), self.parent())
        add_selected_act.setStatusTip(self.tr("Add new bonds between selected atoms"))
        add_selected_act.triggered.connect(self._visualizer.add_bonds_for_selected_atoms)
        bonds_menu.addAction(add_selected_act)

        remove_selected_act = QAction(self.tr("Remove selected"), self.parent())
        remove_selected_act.setStatusTip(self.tr("Remove existing bonds between selected atoms"))
        ...
        toggle_selected_act.setStatusTip(self.tr("Add new or remove existing bonds between selected atoms"))
        ...
        build_dynamically_act = QAction(self.tr("Build dynamically..."), self.parent())
        build_dynamically_act.setStatusTip(self.tr("Build bonds in dynamic mode by adjusting settings"))
        build_dynamically_act.triggered.connect(self._visualizer.rebuild_bonds_dynamic)
        bonds_menu.addAction(build_dynamically_act)

        rebuild_all_act = QAction(self.tr("Rebuild all"), self.parent())
        rebuild_all_act.setStatusTip(self.tr("Remove all current bonds and automatically create a new set of bonds"))
        rebuild_all_act.triggered.connect(self._visualizer.rebuild_bonds)
        bonds_menu.addAction(rebuild_all_act)

        rebuild_default_act = QAction(self.tr("Rebuild default"), self.parent())
        rebuild_default_act.setStatusTip(self.tr("Rebuild bonds automatically using default settings"))
    ...
    def _init_selection_menu(self):
        selection_menu = QMenu(self.tr("Selection"))
        self.addMenu(selection_menu)

        select_all_atoms_act = QAction(self.tr("Select all atoms"), self.parent())
        select_all_atoms_act.triggered.connect(self._visualizer.select_all_atoms)
        selection_menu.addAction(select_all_atoms_act)

        unselect_all_atoms_act = QAction(self.tr("Unselect all atoms"), self.parent())
        ...
        select_toggle_all_atoms_act = QAction(self.tr("Toggle all atoms"), self.parent())
    ...
    def _init_calculate_menu(self):
        calc_menu = QMenu(self.tr("Calculate"))
        self.addMenu(calc_menu)

        calc_interat_distance_act = QAction(self.tr("Interatomic distance"), self.parent())
        calc_interat_distance_act.setStatusTip(self.tr("Distance between last two selected atoms a1-a2"))
        ...
        calc_interat_angle_act = QAction(self.tr("Interatomic angle"), self.parent())
        calc_interat_angle_act.setStatusTip(
            self.tr("Angle between two lines formed by last three selected atoms a1-a2-a3")
        )
        ...
        calc_torsion_angle_act = QAction(self.tr("Torsion angle"), self.parent())
        ...
        calc_oop_angle_act = QAction(self.tr("Out-of-plane angle"), self.parent())
        ...
        calc_auto_parameter_act = QAction(self.tr("Auto parameter"), self.parent())
        calc_auto_parameter_act.setShortcut(QKeySequence(self._keymap.calc_auto_parameter))
        calc_auto_parameter_act.setStatusTip(
            self.tr("Interatomic distance, angle or torsion angle if two, three or four atoms are selected")
        )
        ...
        calc_sel_fragments_act = QAction(self.tr("Selected fragments"), self.parent())
        calc_sel_fragments_act.setStatusTip(
            self.tr("Calculate all geometric parameters for fragments with selected atoms")
        )
    ...
    def _init_cloaking_menu(self):
        cloaking_menu = QMenu(self.tr("Cloaking"))
        self.addMenu(cloaking_menu)

        cloak_selected_act = QAction(self.tr("Cloak all selected"), self.parent())
        cloak_selected_act.triggered.connect(self._visualizer.cloak_selected_atoms)
        cloaking_menu.addAction(cloak_selected_act)

        cloak_not_selected_act = QAction(self.tr("Cloak all not selected"), self.parent())
        ...
        cloak_h_atoms_act = QAction(self.tr("Cloak all H atoms"), self.parent())
        cloak_h_atoms_act.triggered.connect(self._visualizer.cloak_h_atoms)
        cloaking_menu.addAction(cloak_h_atoms_act)

        cloak_notsel_h_atoms_act = QAction(self.tr("Cloak not selected H atoms"), self.parent())
        ...
        cloak_toggle_h_atoms_act = QAction(self.tr("Toggle all H atoms"), self.parent())
        ...
        cloak_at_by_type_act = QAction(self.tr("Cloak atoms by type..."), self.parent())
        ...
        uncloak_all_act = QAction(self.tr("Uncloak all"), self.parent())
    ...
    def _switch_atomic_coordinates_menu(self):
        menu = QMenu(self.tr("Coordinates set"))
        self.addMenu(menu)

        next_atomic_coordinates_act = QAction(self.tr("Next"), self.parent())
        ...
        prev_atomic_coordinates_act = QAction(self.tr("Previous"), self.parent())
