...
### Resources
We use external resource files and load them in runtime using the `QResource.registerResource` function as described in
...
```
This is opposite to the commonly used scheme in PySide, when python modules are created from `qrc` files using `pyside6-rcc` program.
We avoid this due to two issues, (a) loading function is invoked upon module loading and (b) the data are duplicated in python and in Qt library.

### GUI Design
The creation and adjustment of all GUI elements is coded manually.
We do not use Qt Designer and `ui` files bacause of several reasons: (a) this way is not flexible enough and Qt Designer does not allow full manipulation with GUI elements,
(b) the autogenerated by the `uic` utility python code is usually excessive,
(c) import commands in the generated python code do not take into accout paths to resource modules.
